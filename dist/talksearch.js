...
function highlight(hit, key) {
  return (0, _lodash2.default)(hit, `_highlightResult.${key}.value`, (0, _lodash2.default)(hit, key));
}
...
function getThumbnail(hit) {
  return (0, _lodash2.default)(hit, 'video.thumbnails.high.url');
}
...
function renderViews(hit) {
  const rawViewCount = (0, _lodash2.default)(hit, 'video.popularity.views');
...
function renderCaption(hit) {
  ...
  const matchType = (0, _lodash2.default)(hit, '_snippetResult.caption.content.matchLevel', 'none');
  if (matchType === 'none') {
    return '';
  }

  const captionValue = (0, _lodash2.default)(hit, '_snippetResult.caption.content.value');
...
function renderSpeakers(hit) {
  const speakers = (0, _lodash2.default)(hit, 'speakers', []).map(speaker => speaker.name);

  if (speakers.length === 0) {
    return '';
  }
  const renderedSpeakers = speakers.map(speakerName => `<span class="ats-hit--speakerName">${speakerName}</span>`).join(', ');
...
function renderConferenceYear(hit) {
  const conferenceYear = (0, _lodash2.default)(hit, 'conference.year');
  if (!conferenceYear) {
    return '';
  }
...
function renderVideoSubtitle(hit) {
  ...
  const renderedContent = (0, _lodash4.default)([speakers, conferenceYear]).join(' in ');
  return `<div class="ats-hit--videoSubtitle">${renderedContent}</div>`;
}

function hitTemplate(item) {
  const thumbnail = getThumbnail(item);
  const url = (0, _lodash2.default)(item, 'caption.url');
...
var _hitTemplate = __webpack_require__(3);

var _hitTemplate2 = _interopRequireDefault(_hitTemplate);
...
const TalkSearch = {
  templates: {
    hits: {
      item: _hitTemplate2.default
    }
  }
}; /* eslint-disable import/no-commonjs */
exports.default = TalkSearch;
...
const talksearch = __webpack_require__(4);
